...
tf.app.flags.DEFINE_integer('epoches', 256, 'number of epoches')
...
def main(_):
    ...
    with tf.Graph().as_default(), tf.Session() as sess:
        for i in range(FLAGS.xfold):
            ...
            # init loss
            train_loss, train_loss_all, train_acc = .0, np.array([0.] * FLAGS.n_tasks), np.array([0.] * FLAGS.n_tasks)
            ...
            for _n, idx in enumerate(shuffled_batches):
                ...
                _train_loss_all = sess.run(tccnn.loss_all, feed_dict=feed_dict)
                if FLAGS.n_tasks == 1:
                    _train_acc = np.array([_train_acc])
                    _train_loss_all = np.array([_train_loss_all])
                for i in range(FLAGS.n_tasks):
                    train_acc[i] += _train_acc[i]
                    train_loss_all[i] += _train_loss_all[i]
            ...
            if FLAGS.n_tasks == 3:
                print('Epoch {}, loss={:.6f}, loss all = {}, train accuracy = {}, test accuracy '
                      '= {}, test p '
                      '= {}, test r '
                      '= {}, test fscore '
                      '= {}, test auc'
                      '= {}'.format(0, train_loss/num_batches, train_loss_all/num_batches,
...
#                     print (_test_f1,flush=False)
                ...
                if FLAGS.n_tasks == 3:
                    print('Epoch {}, loss={:.6f}, loss all = {}, train accuracy = {}, test accuracy '
                      ...
                      '= {}, test fscore '
                      '= {}, test auc'
                      '= {}'.format(epoch+1, train_loss/num_batches, train_loss_all/num_batches,
                                        train_acc/num_batches, test_acc/num_batches_test,test_p/num_batches_test, test_r/num_batches_test
                                        ,2*test_p*(test_r/num_batches_test)/(test_p+test_r+0.0001), test_auc/num_batches_test),file=output_file, flush=True)
