...
tf.app.flags.DEFINE_string('output', 'Output_sep_task_all3_b64_lstm2.txt', 'output_file')
tf.app.flags.DEFINE_integer('xfold', 10, 'number of folds')
tf.app.flags.DEFINE_string('save_to', 'saved_models_lstm', 'folder for saved models')
...
def main(_):
    ...
    logfile = open(os.path.join(HOME, 'saved_models_lstm', 'xylog'),"w")
    if FLAGS.n_tasks == 1:
    ...
    attack, aggression, toxicity = data[:, -3], data[:, -2], data[:, -1]
